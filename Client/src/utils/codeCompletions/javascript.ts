...
const FRAME_APIS: Record<string, Completion[]> = {
  ...
  react: [
    {
      label: 'useState',
      apply: 'const [$0, set${0/(.*)/${1:/capitalize}/}] = useState($1)',
      boost: 10,
    ...
    { label: 'useRef', apply: 'useRef($0)', boost: 9, info: 'React ref hook' },
    {
      ...
      boost: 8,
      info: 'React layout effect hook',
    },
    {
      label: 'useImperativeHandle',
      apply: 'useImperativeHandle($0, () => {\n\t$1\n}, [$2])',
      boost: 8,
      info: 'React imperative handle hook',
    },
...
export const jsCompletions = (context: CompletionContext, projectContext?: any) => {
  ...
  if (identifierMatch) {
    ...
    return {
      ...
      options: [
        ...JS_KEYWORDS.filter((k) => k.label.startsWith(prefix) && !k.label.endsWith(':')),
        ...userVars
          .filter((v: VariableInfo) => v.name && v.name.startsWith(prefix))
          .map((v) => ({
            label: v.name,
            type: v.type === 'function' ? 'function' : v.type === 'class' ? 'class' : 'variable',
            detail: v.type,
  ...
  if (memberExprMatch) {
    ...
    return {
      ...
      options: objCompletions.filter((c) => c.label.startsWith(prop)),
      filter: false,
    };
  }
  // 处理普通标识符补全
  ...
  function getObjectCompletions(obj: string): Completion[] {
    const OBJ_PROPS: Record<string, Completion[]> = {
      ...
      document: [{ label: 'querySelector', type: 'method', apply: "querySelector('$0')" }],
    };
