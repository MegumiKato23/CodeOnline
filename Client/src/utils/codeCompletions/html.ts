...
const JSX_ATTRIBUTES: Completion[] = [
  ...
  { label: 'htmlFor', apply: 'htmlFor="$0"', boost: 8, info: 'React label for attribute' },
  {
    ...
    apply: 'dangerouslySetInnerHTML={{ __html: $0 }}',
    boost: 7,
    info: 'React raw HTML insertion',
  },
...
export function htmlCompletions(context: CompletionContext, projectContext?: any) {
  ...
  if (tagOpenMatch) {
    return {
      ...
      options: [
        ...
        ...customTags
          .filter((tag) => tag.startsWith(tagOpenMatch[1].toLowerCase()))
          .map((label) => ({ label, type: 'tag' })),
      ].map((tag) =>
        isJsx
          ? {
              ...tag,
  ...
  if (attrMatch) {
    return {
      ...
      options: HTML_ATTRIBUTES.filter((attr) => attr.label.startsWith(attrMatch[1].toLowerCase())),
      filter: false,
    };
  }
